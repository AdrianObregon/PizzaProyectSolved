            ...
            if (json["success"] === "0") {
                ...
                if (json.toppings.length > 0) {
                    ...
                    $.each(json.toppings, function(key, value) {
                        $("#listToppings").append("<li class='list-group-item d-flex justify-content-between align-items-center'><span>" + value + 
                                "</span></li><button type='button' onClick='deleteTopping("+key+")'>Delete it!</button>");
                    });
                    $('p.hasToppings').show();
                    $('p.isEmpty').hide();
                } else {
